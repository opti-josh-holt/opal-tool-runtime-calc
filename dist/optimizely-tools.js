...
async function getExperimentResults(params) {
    ...
    try {
        ...
        return {
            ...
            start_time: results.start_time ? formatDate(results.start_time) : undefined,
            ...
            variations: results.results?.map(variation => ({
                ...
                improvement: variation.improvement ? Math.round(variation.improvement * 10000) / 100 : undefined,
                statistical_significance: variation.statistical_significance ? Math.round(variation.statistical_significance * 100) / 100 : undefined,
                is_winner: winningVariation ? variation.variation_id === winningVariation.variation_id : false
            })) || [],
            summary: {
...
async function createExperiment(params) {
    ...
    try {
        ...
        const experimentData = {
            name,
            description: description || `Experiment created via Opal on ${new Date().toLocaleDateString()}`,
